---
description: Project threat model — show, initialise, build, refresh, export
...
`--threat-model-only` path. `show`, `init`, `export`, and `sync` call the
existing project threat-model path. Do not manually recreate the logic in the
slash command.
